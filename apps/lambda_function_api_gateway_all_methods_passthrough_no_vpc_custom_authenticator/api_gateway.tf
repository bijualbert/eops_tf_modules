...
# swagger
resource "aws_api_gateway_resource" "swagger_parent" {
  parent_id   = "${aws_api_gateway_rest_api.app.root_resource_id}"
  rest_api_id = "${aws_api_gateway_rest_api.app.id}"
  path_part   = "${var.swagger_resource_path}"
  count       = "${var.swagger_with_api_key}"
}

resource "aws_api_gateway_resource" "swagger" {
  parent_id   = "${aws_api_gateway_resource.swagger_parent.id}"
  rest_api_id = "${aws_api_gateway_rest_api.app.id}"
  path_part   = "{proxy+}"
  count       = "${var.swagger_with_api_key}"
}
